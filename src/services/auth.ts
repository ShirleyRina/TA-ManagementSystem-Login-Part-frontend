...
const register = (username: string, email: string, password: string) => {
  return axios.post(USER_API_URL + "/user/signup", {
    username,
    email,
    password,
  });
...
const logout = () => {
  ...
  return axios.post(USER_API_URL + "/user/logout").then((response) => {
    return response.data;
  });
};
...
const forgotPassword = (email: string) => {
  return axios.post(USER_API_URL + "/api/password-reset/link", {
    email,
  });
};
...
const resetPassword = (token: string, newPassword: string) => {
  return axios.post(USER_API_URL + "/api/password-reset/confirm", {
    token,
